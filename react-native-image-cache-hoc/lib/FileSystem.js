...
export class FileSystem {
  ...
  constructor(cachePruneTriggerLimit = null, fileDirName = null) {
    ...
    fileDirName = fileDirName || 'hepimusicimagecache'; // Namespace local file writing to this folder.
  ...
  async getLocalFilePathFromUrl(url, permanent) {
    ...
    if (exists[0]) {
      ...
      console.log("About to fetch file")
  ...
  async fetchFile(url, permanent = false, fileName = null, clobber = false) {
    ...
    try {
      ...
      console.log("fromUrl: "+url+" toFile: "+path+" RNFS.DocumentDirectoryPath: "+RNFS.DocumentDirectoryPath);
  ...
  async pruneCache() {
    ...
    let lstat = await RNFS.lstat(this.baseFilePath + 'cache');
    ...
    if (currentCacheSize > this.cachePruneTriggerLimit) {
      ...
      while (overflowSize > 0 && lstat.length) {
        ...
        if (!FileSystem.cacheLock[blobStatObject.filename] && this._validatePath('cache/' + blobStatObject.filename)) {
          ...
          RNFS.unlink(this.baseFilePath + 'cache/' + blobStatObject.filename);
        }
  ...
  async unlink(path) {
    ...
    try {
      ...
      await RNFS.unlink(pathLib.resolve(this.baseFilePath + path));
      return true;
    } catch (error) {
      return false;
    }
